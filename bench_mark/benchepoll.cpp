...
typedef enum { PROTOCOL_TCP = 1, PROTOCOL_UDP } PROTOCOL;
...
uint64_t msTime() {
  ...
  if (_tv.tv_sec == 0) gettimeofday(&_tv, NULL);
...
void touchTime() { gettimeofday(&_tv, NULL); }
...
void parseOptions(int argc, char** argv) {
  int opt = 0;
  while ((opt = getopt_long(argc, argv, ":c:n:h:p:kru", longopts, NULL)) !=
         -1) {
    switch (opt) {
      case 'c':
      ...
      case 'r':
        ...
        break;
      default:
        cout << "Usage: benchmark [-h <host>] [-p <port>] [-c <clients>] [-n "
                "<requests]> [-k] [-r] [-u]"
             << endl;
        cout << " -h <host> Server ip (default 127.0.0.1)" << endl;
        cout << " -p <port> Server port (default 5113)" << endl;
        cout << " -c <clients> Number of parallel connections (default 5)"
             << endl;
...
int recvFromServer(void* cl) {
  ...
  if (c->latency < 0) c->latency = usTime() - c->startTime;
  ...
  while (1) {
    int received = read(c->fd, c->recvBuf + c->recvBufLen,
                        sizeof(c->recvBuf) - c->recvBufLen);
    if (received < 0) {
      if (errno == EINTR) {
        continue;
  ...
  if (c->recvBufLen > MAX_RECV_BUF_LEN) {
    cout << "client recv buffer is full, Please make sure that is enough"
         << endl;
    exit(-1);
  }
...
int sendToServer(void* cl) {
  ...
  while (1) {
    sended =
        write(c->fd, c->sendBuf + c->sendedLen, c->sendBufLen - c->sendedLen);
    if (sended <= 0) {
      if (errno == EINTR) {
        continue;
      } else if (errno == EWOULDBLOCK || errno == EAGAIN) {
...
int createClient() {
  ...
  // set noblock
  if (setNonBlock(fd) < 0) goto CREATE_FAIL;
  ...
  if (addEvent(c, WRITEABLE, sendToServer, &config) < 0) goto CREATE_FAIL;
...
void showThroughPut() { uint64_t totalTime = msTime() - config.startTime; }
...
void showReport() {
  ...
  cout << config.doneRequests << " completed in " << totalTime / 1000
       << " seconds" << endl;
  ...
  cout << "test start time:" << (config.startTime / 1000) / 3600 << ":"
       << ((config.startTime / 1000) % 3600) / 60 << ":"
       << ((config.startTime / 1000) % 3600) % 60 * 0.001;
  cout << "test end time:" << (config.endTime / 1000) / 3600 << ":"
       << ((config.endTime / 1000) % 3600) / 60 << ":"
       << ((config.endTime / 1000) % 3600) % 60 * 0.001;
  cout << "50% <= " << config.latency[config.doneRequests / 2] / 1000
       << " milliseconds" << endl;
  cout << "60% <= " << config.latency[config.doneRequests / 10 * 6] / 1000
       << " milliseconds" << endl;
  cout << "70% <= " << config.latency[config.doneRequests / 10 * 7] / 1000
       << " milliseconds" << endl;
  cout << "80% <= " << config.latency[config.doneRequests / 10 * 8] / 1000
       << " milliseconds" << endl;
  cout << "90% <= " << config.latency[config.doneRequests / 10 * 9] / 1000
       << " milliseconds" << endl;
  cout << "100% <= " << config.latency[config.doneRequests - 1] / 1000
       << " milliseconds" << endl;

  avg_lat = total_lat / config.doneRequests;

  cout << (config.endTime - config.startTime) << " total milliseconds used"
       << endl;
...
void benchmark() {
  ...
  while (config.el->stop) {
    int num;
    num = epoll_wait(config.el->epfd, config.el->events,
                     config.needClientNum + 1, 2000);
    ...
    for (int i = 0; i < num; i++) {
      ...
      if (e->events & (EPOLLERR | EPOLLHUP)) {
        if (++epoll_fail > MAX_FAIL_COUNT) {
          cout << "the epoll fail count is more than " << MAX_FAIL_COUNT
               << endl;
...
int main(int argc, char** argv) {
  ...
  if (config.needClientNum > config.needRequestNum) {
    cout << "-n cannot be smaller than -c\n";
    return -1;
  }
